    ...
    <div className="space-y-6">
      ...
      <Card className="border-white/10 bg-white/[0.02]">
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-gray-400">Delegate your voting power to another member. You can reclaim it at any time by delegating back to yourself.</p>
          <div className="flex gap-3">
            <Input placeholder="Delegate address (0x...)" value={delegateAddress} onChange={(e) => setDelegateAddress(e.target.value)} className="border-white/10 bg-white/5 font-mono text-sm" />
            <Button disabled={!isConnected || !isAddress(delegateAddress) || isDelegating || isDelegateConfirming} onClick={handleDelegate} className="bg-indigo-500 hover:bg-indigo-400">
              {isDelegating ? "Confirm..." : isDelegateConfirming ? "Delegating..." : "Delegate"}
            </Button>
          </div>
          {isConnected && address && <Button variant="outline" size="sm" onClick={() => setDelegateAddress(address)} className="border-white/10 text-xs">Delegate to myself</Button>}
      ...
      <Card className="border-white/10 bg-white/[0.02]">
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12">
            <svg className="h-12 w-12 text-gray-600" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" />
            </svg>
            <p className="mt-4 text-gray-400">Member data is indexed from on-chain events.</p>
          </div>
        </CardContent>
      </Card>
    </div>
