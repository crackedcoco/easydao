...
export default function TreasuryPage() {
  ...
  // C4 fix: look up the timelock (treasury) address from the governor
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-white/10 bg-white/[0.02]">
          <CardContent className="p-6">
            ...
            <p className="mt-1 text-sm text-gray-500">ETH</p>
          </CardContent>
        </Card>
      ...
      <Card className="border-white/10 bg-white/[0.02]">
        ...
        <CardContent className="space-y-2">
          <p className="text-sm text-gray-400">
            The treasury is controlled by the DAO&apos;s timelock contract.
            Funds can only be moved through approved proposals.
          </p>
          {timelockAddress && (
            <>
              <p className="break-all font-mono text-xs text-gray-500">{timelockAddress}</p>
              <a
                href={`https://basescan.org/address/${timelockAddress}`}
                target="_blank"
